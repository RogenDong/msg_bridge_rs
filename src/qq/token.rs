...
use super::{CfgErr, CfgKind, OprKind, RicqClient, FILE_TOKEN_BIN, FILE_TOKEN_JSON};
...
/// - `dir` 保存目录
/// - `client` 客户端
pub(crate) async fn upd_token_bin(dir: &Path, client: &RicqClient) -> Result<(), CfgErr> {
    use tokio::io::AsyncWriteExt;
    exs!(dir);
    let path = dir.join(FILE_TOKEN_BIN);
    ...
    let token = token_to_bytes(&token);
    if let Err(e) = file.write_all(&token).await {
